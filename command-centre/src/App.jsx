...
const TABS = [
  { id:'gmail',    label:'📧 Gmail',          badge:null },
  { id:'slips',    label:'🆘 Help Slips',      badge:null },
  { id:'mentions', label:'💬 My Mentions',     badge:null },
  { id:'scheduler',label:'📅 Daily Scheduler', badge:null },
    ...
    <div style={{ background:T.bg, height:'100vh', fontFamily:"'Inter', system-ui, sans-serif", color:T.text, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ background:T.surface, borderBottom:`1px solid ${T.border}`, padding:'0 20px', display:'flex', alignItems:'center', justifyContent:'space-between', height:48, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:28, height:28, borderRadius:8, background:`linear-gradient(135deg,${T.violet},${T.gold})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:800, color:'#fff' }}>✦</div>
          <span style={{ fontWeight:800, fontSize:15, letterSpacing:0.2 }}>MySoul Command Centre</span>
          <span style={{ fontSize:11, color:T.textDim, background:'rgba(255,255,255,0.05)', borderRadius:6, padding:'2px 8px' }}>W8 · Y3 · LIVE</span>
        </div>
        ...
        {TABS.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{ background:'transparent', border:'none', borderBottom:active?`2px solid ${T.gold}`:'2px solid transparent', padding:'12px 18px', color:active?T.gold:T.textMid, cursor:'pointer', fontSize:13, fontWeight:active?700:400, display:'flex', alignItems:'center', gap:6, transition:'all 0.15s', whiteSpace:'nowrap', flexShrink:0 }}>
              {tab.label}
              {tab.badge && (
                <span style={{ background:T.goldDim, color:T.gold, borderRadius:6, padding:'1px 7px', fontSize:10, fontWeight:700, border:`1px solid ${T.gold}44` }}>{tab.badge}</span>
              )}
            </button>
          )
        })}
