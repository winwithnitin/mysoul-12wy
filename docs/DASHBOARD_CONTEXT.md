...
- V5 shifted-student rule: if the same name, email, or phone appears in different SUPER/RGM batches, highlight these shifted students separately with an open/close section. The latest batch is the active follow-up row and gets a `Shifted (Last batch)` tag; older batch rows stay visible in verification history but are kept out of collection calendar and delayed-payment action lists to avoid repeated dues.
