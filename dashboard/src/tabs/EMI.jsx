...
function V1Content({ students }) {
  ...
  const filtered = students.filter(s =>
    (prog==='ALL' || s.program===prog) && (batch==='ALL' || s.batch===batch)
  );
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:28 }}>
          {[{label:'SUPER',m:superM,color:'var(--tarot)'},{label:'RGM',m:rgmM,color:'var(--reiki)'}].map(({label,m,color})=>(
            <div key={label} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderTop:`3px solid ${color}`, borderRadius:12, padding:'16px 18px' }}>
              ...
              {[['Students',num(filtered.filter(s=>s.program===label).length)],['Total outstanding',inr(m.outstanding)],['This month expected',inr(m.monthExpected)],['This month received',inr(m.monthReceived)],['This month gap',inr(m.monthGap)],['Overdue EMIs',num(m.overdueList.length)]].map(([l,v])=>(
...
export default function EMI() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [emiData, salesData] = await Promise.all([
        loadEMIData(),
        loadSalesData(),
      ]);
      setData(emiData);
      ...
      console.warn('EMI demo:', e.message);
