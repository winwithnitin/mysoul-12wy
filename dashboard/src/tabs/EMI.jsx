...
import { loadEMIData, getEMISample } from '../utils/sheets.js';
...
function calcMetrics(students) {
  ...
  for (const s of students) {
    ...
    for (const emi of (s.emis || [])) {
      if (emi.plannedDate >= ms && emi.plannedDate <= me && !emi.actualDate)
        monthExpected += emi.plannedAmt || 0;
      if (emi.actualDate >= ms && emi.actualDate <= me)
        monthReceived += emi.actualAmt || 0;
...
export default function EMI() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setData(await loadEMIData()); setDemo(false); }
    catch (e) { console.warn('EMI demo:', e.message); setData(getEMISample()); setDemo(true); }
  ...
  const all    = calcMetrics(filtered);
  const superM = calcMetrics(filtered.filter(s => s.program === 'SUPER'));
  const rgmM   = calcMetrics(filtered.filter(s => s.program === 'RGM'));

  const batchRows = allBatches.map(b => {
    const list = filtered.filter(s => s.batch === b);
    if (!list.length) return null;
    const m = calcMetrics(list);
    return { b, prog: list[0]?.program, count: list.length, ...m };
  }).filter(Boolean);
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 24px', borderBottom:'1px solid var(--border)', background:'var(--surface2)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div style={{ display:'flex', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:8, overflow:'hidden' }}>
            {['ALL','SUPER','RGM'].map(p => (
              <button key={p} onClick={() => setProg(p)} style={{ border:'none', borderRadius:0, padding:'4px 12px', fontSize:12, fontWeight:500, background: prog===p ? 'var(--tarot)' : 'transparent', color: prog===p ? '#fff' : 'var(--text3)' }}>{p}</button>
            ))}
          </div>
          <select value={batch} onChange={e => setBatch(e.target.value)}
            style={{ background:'var(--surface)', border:'1px solid var(--border)', color:'var(--text)', padding:'4px 10px', borderRadius:8, fontSize:12 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:1, background:'var(--border)', marginBottom:24 }}>
        {[
          { label:'Total outstanding',    value: inr(all.outstanding),  color: all.outstanding > 0 ? 'var(--warning)' : 'var(--text)' },
          { label:'This month expected',  value: inr(all.monthExpected), color: null },
          { label:'This month received',  value: inr(all.monthReceived), color:'var(--success)' },
          { label:'This month gap',       value: inr(all.monthGap),      color: all.monthGap > 0 ? 'var(--danger)' : 'var(--success)' },
          { label:'Overdue EMIs',         value: num(all.overdueList.length), color: all.overdueList.length > 0 ? 'var(--danger)' : 'var(--success)' },
      ...
      <div style={{ padding:'0 24px 32px' }}>
        ...
        {/* SUPER vs RGM */}
        {eyebrow('Program overview — month to date')}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:28 }}>
          {[{label:'SUPER', m:superM, color:'var(--tarot)'}, {label:'RGM', m:rgmM, color:'var(--reiki)'}].map(({ label, m, color }) => (
            <div key={label} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderTop:`3px solid ${color}`, borderRadius:12, padding:'16px 18px' }}>
              ...
              {[
                ['Students',             num(filtered.filter(s => s.program === label).length)],
                ['Total outstanding',    inr(m.outstanding)],
                ['This month expected',  inr(m.monthExpected)],
                ['This month received',  inr(m.monthReceived)],
                ['This month gap',       inr(m.monthGap)],
                ['Overdue EMIs',         num(m.overdueList.length)],
              ].map(([lbl, val]) => (
        ...
        {/* Batch breakdown */}
        {eyebrow('Batch breakdown')}
        <div style={tableWrap}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr>{['Batch','Program','Students','Outstanding','Mth Expected','Mth Received','Gap','Overdue'].map((h,i) => <th key={h} style={i<2?thL:th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {batchRows.length === 0
                ? <tr><td colSpan={8} style={{...td(),textAlign:'center',padding:'2rem'}}>No data.</td></tr>
                : batchRows.map(r => (
                  <tr key={r.b+r.prog}>
        ...
        {/* Overdue students */}
        {eyebrow(`Overdue EMIs${all.overdueList.length > 0 ? ` — ${all.overdueList.length} unpaid` : ''}`)}
        <div style={{...tableWrap, border:`1px solid ${all.overdueList.length>0?'var(--danger)':'var(--border)'}`}}>
          {all.overdueList.length === 0
            ...
            : <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                ...
                <tbody>
                  {all.overdueList.sort((a,b)=>b.daysOverdue-a.daysOverdue).map((s,i)=>(
                    <tr key={i}>
